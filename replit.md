...
- June 17, 2025: **PDF INVOICE SYSTEM**: Implemented comprehensive server-side PDF invoice generation with jsPDF library. Created professional invoice templates featuring Proply branding, tiered pricing breakdowns, agency details, and payment status. Added secure download endpoints with proper authentication and permission checks. PDF invoices available in both agency details modal and branch admin settings with disabled downloads for upcoming billing periods. Logo displays with proper aspect ratio (3.8:1) and uses official Proply blue color (#1ba2ff).
