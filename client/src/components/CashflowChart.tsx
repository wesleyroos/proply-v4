...
export default function CashflowChart({ netOperatingIncome }: CashflowChartProps) {
  ...
  const generateChartData = (isShortTerm: boolean) => {
    return years.map((year, index) => {
      const yearKey = `year${year}` as keyof typeof netOperatingIncome;
      const annualCashflow = isShortTerm ? netOperatingIncome[yearKey].annualCashflow : netOperatingIncome[yearKey].value;
      ...
      for (let i = 0; i <= index; i++) {
        ...
        cumulativeCashflow += isShortTerm ? netOperatingIncome[yKey].annualCashflow : netOperatingIncome[yKey].value;
      }
  ...
  const renderChart = (data: any[]) => {
    ...
    const minValue = Math.min(
      ...data.map(d => Math.min(d['Annual Cashflow'], d['Cumulative Cashflow']))
    );
    const maxValue = Math.max(
      ...data.map(d => Math.max(d['Annual Cashflow'], d['Cumulative Cashflow']))
    );
  ...
  return (
    <Tabs defaultValue="short-term" className="w-full">
      ...
      <TabsContent value="short-term">
        {renderChart(generateChartData(true))}
      </TabsContent>
      <TabsContent value="long-term">
        {renderChart(generateChartData(false))}
      </TabsContent>
    </Tabs>
  );
}
