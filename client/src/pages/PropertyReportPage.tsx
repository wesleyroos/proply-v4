...
function MiniCard({
  ...
  return (
    <div className="bg-slate-50 border border-slate-100 rounded-xl p-4">
      <div className="text-[9px] font-semibold uppercase tracking-widest text-slate-400 mb-1.5">
        {label}
      </div>
      <div
        className="text-[15px] font-bold leading-tight"
        style={{ color: valueColor || "#0d1b2a" }}
      >
        {value}
      </div>
...
export default function PropertyReportPage() {
  ...
  const { data, isLoading, isError } = useQuery<ReportData>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/propdata-reports/preview-data/${reportId}`);
  ...
  if (str && price) {
    const p25 = str.percentile25?.annual
      ? ((str.percentile25.annual / price) * 100).toFixed(1)
      : null;
    const p75 = str.percentile75?.annual
      ? ((str.percentile75.annual / price) * 100).toFixed(1)
      : null;
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <section className="relative bg-slate-900 overflow-hidden" style={{ minHeight: 340 }}>
        ...
        <div
          ...
          style={{
            background: `linear-gradient(135deg, ${accentColor}55 0%, #0d1b2a99 100%)`,
          }}
        />
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 py-12">
          ...
          {p.price && (
            <div className="mb-6">
              <div className="text-[10px] font-semibold uppercase tracking-widest text-white/50 mb-1">
                Asking Price
              </div>
        ...
        <section className="bg-white border-b border-slate-200">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 py-5 grid grid-cols-2 sm:grid-cols-3 divide-x divide-slate-100 gap-y-4">
            ...
            {strYieldRange && (
              <div className="px-4">
                <StatPill
                  label="STR Gross Yield"
                  value={strYieldRange}
                  sub="Short-term / Airbnb"
                  accent="#16a34a"
                />
              </div>
            )}
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-100">
                        ...
                        <th className="text-center px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
                          Formula
                        </th>
                        <th className="text-right px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
                          Valuation
                        </th>
                      </tr>
                    </thead>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8">
            ...
            {ltr && (
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                  <MiniCard
                    label="Monthly Range"
                    value={`${fmt(ltr.minRental)} – ${fmt(ltr.maxRental)}`}
                  />
                  <MiniCard
                    label="Gross Yield Range"
                    value={`${ltr.minYield ?? "N/A"}% – ${ltr.maxYield ?? "N/A"}%`}
                    valueColor="#16a34a"
                  />
                  <MiniCard
                    label="Annual Revenue"
                    value={`${fmt(ltr.minRental ? ltr.minRental * 12 : null)} – ${fmt(ltr.maxRental ? ltr.maxRental * 12 : null)}`}
                  />
                  <MiniCard label="Strategy" value="Long-term let" sub="12-month lease" />
                </div>
                {ltr.reasoning && (
                  <p className="text-sm text-slate-500 leading-relaxed">{ltr.reasoning}</p>
                )}
              </div>
            )}
            ...
            {str && (
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                  {str.occupancy && (
                    <MiniCard label="Avg Occupancy" value={`${str.occupancy}%`} />
                  )}
                  {[
                        ...
                        {[
                          ...
                          .map((l, i) => {
                            ...
                            return (
                              <tr
                                key={l.key}
                                className={`border-b border-slate-50 last:border-0 ${isMedian ? "bg-blue-50/60" : ""}`}
                              >
                                <td className={`px-4 py-3 ${isMedian ? "font-semibold" : ""}`}>
                                  {l.label}
                                </td>
                                ...
                                <td className={`px-4 py-3 text-right font-bold ${isMedian ? "" : "text-slate-700"}`}
                                  style={isMedian ? { color: accentColor } : {}}>
                                  {fmt(d.annual)}
                                </td>
                              </tr>
                            );
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8">
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        <MiniCard
                          label="Deposit"
                          value={fmt(Math.round(fin.depositAmount))}
                          sub={`${fin.depositPercentage}% of purchase price`}
                        />
            ...
            {ym && (
              <div className="mb-8">
                    ...
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-100">
                        ...
                        {[1, 2, 3, 4, 5, 10, 20].map((y) => (
                          <th
                            key={y}
                            className="text-right px-3 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400"
                          >
                            Y{y}
                          </th>
                        ))}
                      ...
                      {[
                        ...
                        <tr key={key} className="border-b border-slate-50 last:border-0">
                          ...
                          {[1, 2, 3, 4, 5, 10, 20].map((y) => {
                            ...
                            const isHighlight =
                              (key === "equityBuildup" || key === "remainingBalance") && y >= 10;
                            return (
                              <td
                                key={y}
                                className={`px-3 py-3 text-right text-xs ${isHighlight ? "font-bold" : "text-slate-600"}`}
                                style={isHighlight ? { color: accentColor } : {}}
                              >
                                {fmt(v)}
                              </td>
                            );
                ...
                <p className="text-xs text-slate-400 mb-4">
                  Loan paydown and equity accumulation over the mortgage term
                </p>
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={equityChartData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                    ...
                    <Legend
                      ...
                      formatter={(value) =>
                        value === "equity" ? "Equity Built" : "Remaining Balance"
                      }
                    />
                    <Area
                      type="monotone"
                      ...
                      strokeWidth={2.5}
                      fill="url(#equityGrad)"
                      dot={{ r: 3, fill: "white", stroke: accentColor, strokeWidth: 2 }}
                    />
                    <Line
                      type="monotone"
                      dataKey="balance"
            ...
            {rd?.cashflowAnalysisData?.revenueGrowthTrajectory && (() => {
              ...
              return (
                <div className="mb-8">
                  <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
                    Revenue Projections — 8% Annual Growth
                  </h3>
                  <p className="text-xs text-slate-400 mb-4">
                    Projected annual revenue and gross yields over five years
                  </p>
                  <div className="overflow-x-auto rounded-xl border border-slate-100">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-slate-50 border-b border-slate-100">
                          ...
                          {["Year 1", "Year 2", "Year 3", "Year 4", "Year 5"].map((h) => (
                            <th key={h} className="text-right px-3 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
                              {h}
                            </th>
                          ))}
                        </tr>
                          ...
                          return (
                            <>
                              <tr key={`${key}-rev`} className={`border-b border-slate-50 ${isMedian ? "bg-blue-50/40" : ""}`}>
                                <td className={`px-4 py-2.5 ${isMedian ? "font-semibold" : "text-slate-600"}`}>
                                  {label} — Revenue
                                </td>
                                {[1, 2, 3, 4, 5].map((y) => (
                        ...
                        {traj.longTerm && (
                          <>
                            <tr className="border-b border-slate-50">
                              ...
                              {[1, 2, 3, 4, 5].map((y) => (
                                <td key={y} className="px-3 py-2.5 text-right text-xs text-slate-600">
                                  {fmt(Math.round(traj.longTerm[`year${y}`]?.revenue || 0))}
                                </td>
                              ))}
                            </tr>
                            <tr>
                              ...
                              {[1, 2, 3, 4, 5].map((y) => (
                                <td key={y} className="px-3 py-2.5 text-right text-xs text-slate-600">
                                  {(traj.longTerm[`year${y}`]?.grossYield || 0).toFixed(1)}%
                                </td>
                              ))}
                            </tr>
                          </>
            ...
            {rd?.annualPropertyAppreciationData && (() => {
              ...
              const keys = ["year1", "year2", "year3", "year4", "year5", "year10", "year20"].filter(
                (k) => appr.yearlyValues?.[k]
              );
              return (
                <div>
                  <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-slate-50 border-b border-slate-100">
                            ...
                            {keys.map((k) => (
                              <th key={k} className="text-right px-3 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
                                Year {k.replace("year", "")}
                              </th>
                            ))}
                          </tr>
                          ...
                          <tr>
                            ...
                            {keys.map((k) => {
                              ...
                              return (
                                <td
                                  key={k}
                                  className={`px-3 py-3 text-right text-xs ${isLong ? "font-bold" : "text-slate-600"}`}
                                  style={isLong ? { color: accentColor } : {}}
                                >
                  ...
                  {appr.reasoning && (
                    <p className="text-sm text-slate-500 leading-relaxed">{appr.reasoning}</p>
                  )}
                </div>
              );
            })()}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8">
                ...
                <div className="space-y-2.5">
                  ...
                  {p.agentPhone && (
                    <div className="flex items-center gap-2.5 text-sm text-slate-700">
                      ...
                      <a href={`tel:${p.agentPhone}`} className="hover:underline">
                        {p.agentPhone}
                      </a>
                    </div>
                  )}
                  {p.agentEmail && (
                    <div className="flex items-center gap-2.5 text-sm text-slate-700">
                      ...
                      <a href={`mailto:${p.agentEmail}`} className="hover:underline">
                        {p.agentEmail}
                      </a>
                    </div>
                  )}
              ...
              <div className="space-y-2.5">
                ...
                {p.lastModified && (
                  <div className="flex items-center gap-2.5 text-sm text-slate-700">
                    ...
                    Last Updated:{" "}
                    {new Date(p.lastModified).toLocaleDateString("en-ZA")}
                  </div>
                )}
              </div>
            </div>
          ...
          <p className="text-[11px] text-slate-400 mt-6 pt-6 border-t border-slate-100">
            Report generated {dateStr} by Proply Tech (Pty) Ltd. Valid for 30 days from
            generation date.
          </p>
        </div>
      </main>
