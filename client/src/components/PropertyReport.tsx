...
export const PropertyReport = forwardRef<HTMLDivElement, Props>(({ 
  open, 
  onOpenChange, 
  data, 
  companyLogo 
    ...
    <div ref={ref} className="bg-white p-8">
      ...
      <div className="flex justify-between items-center border-b pb-6">
        <div>
          ...
          <p className="text-gray-600">{data?.propertyDetails?.address ?? 'Address not available'}</p>
      ...
      <div className="grid grid-cols-3 gap-6 mt-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-gray-500 text-sm">{data?.financialMetrics?.depositPercentage ?? 0}% deposit</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-gray-500 text-sm">{data?.financialMetrics?.interestRate ?? 0}% interest</p>
        </div>
      </div>
        ...
        <div className="bg-blue-50 p-6 rounded-lg">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-green-600">{data?.performance?.shortTermGrossYield?.toFixed(2) ?? '0'}% Yield</p>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-semibold">{data?.performance?.annualOccupancy ?? 0}%</p>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-purple-50 p-6 rounded-lg">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-green-600">{data?.performance?.longTermGrossYield?.toFixed(2) ?? '0'}% Yield</p>
            </div>
            <div>
              <p className="text-gray-600">Monthly Rental</p>
              <p className="font-semibold">
                {formatCurrency((data?.performance?.longTermAnnualRevenue ?? 0) / 12)}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-4 gap-6 mt-6">
        <div className="bg-green-50 p-6 rounded-lg">
          ...
          <p className="text-2xl font-bold">
            {formatCurrency(data?.netOperatingIncome?.year20?.netWorthChange)}
          </p>
          <p className="text-green-600">
            {(((data?.netOperatingIncome?.year20?.netWorthChange ?? 0) /
              ((data?.propertyDetails?.purchasePrice ?? 0) * 0.2)) * 100).toFixed(0)}% ROI
          </p>
        </div>
        <div className="bg-blue-50 p-6 rounded-lg">
          ...
          <p className="text-blue-600">
            {(((assetData[6].Value / (data?.propertyDetails?.purchasePrice ?? 1)) - 1) * 100).toFixed(0)}% Growth
          </p>
        </div>
        <div className="bg-purple-50 p-6 rounded-lg">
