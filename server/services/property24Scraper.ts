...
/**
 * Load suburb codes from the generated file if it exists,
 * otherwise use a fallback hardcoded map
 */
function loadSuburbCodes(): Record<string, string> {
  // Path to the generated suburb code map
  const generatedMapPath = path.join(__dirname, '..', 'data', 'cape_town_suburb_codes.ts');
  
  // Check if the generated map exists
  let useGeneratedMap = false;
  try {
    if (fs.existsSync(generatedMapPath)) {
      useGeneratedMap = true;
      console.log('Using generated suburb code map from data/cape_town_suburb_codes.ts');
    }
  } catch (error) {
    console.error('Error checking for generated suburb code map:', error);
  }
  
  // If generated map exists, import and use it
  if (useGeneratedMap) {
    try {
      // We can't dynamically import TS files, so instead we'll read the file content
      // and parse the map directly (simplified approach)
      const fileContent = fs.readFileSync(generatedMapPath, 'utf8');
      
      // Extract the suburb code assignments from the file
      const codeMap: Record<string, string> = {};
      const lines = fileContent.split('\n');
      
      for (const line of lines) {
        // Look for lines with pattern: 'suburb': 'code',
        const match = line.match(/'([^']+)'\s*:\s*'(\d+)'/);
        if (match && match.length >= 3) {
          const [_, suburb, code] = match;
          codeMap[suburb] = code;
        }
      }
      
      console.log(`Loaded ${Object.keys(codeMap).length} suburb codes from generated map`);
      return codeMap;
    } catch (error) {
      console.error('Error loading generated suburb code map:', error);
      console.log('Falling back to hardcoded suburb codes');
    }
  }
  
  // Fallback to hardcoded map
  console.log('Using hardcoded suburb code map');
  return {
    // Central Cape Town
    ...
    // Atlantic Seaboard
    'green point': '11017', // Updated from 9146 to correct code
    'sea point': '11021', // Updated from 9161 to correct code
    'mouille point': '9153',
    'three anchor bay': '9165',
    'camps bay': '11014', // Updated from 9136 to correct code
    'clifton': '9140',
    'bantry bay': '9130',
    
    // Southern Suburbs
    'woodstock': '10164',
    'observatory': '10157', // Updated from 9155 to correct code
    'salt river': '9160',
    'kenilworth': '9148',
    'claremont': '9141',
    'rondebosch': '8682', // Updated from 9158 to correct code
    'newlands': '9154',
    'bishop\'s court': '9132',
    'constantia': '11742', // Updated from 9142 to correct code
    'tokai': '9167',
 ...
 * 
 * This uses dynamically loaded suburb codes from the data directory
 * if available, or falls back to a hardcoded set of codes.
 * 
 * To update the suburb codes, run:
 * npx tsx server/scripts/extract_cape_town_suburbs.ts
 * 
 * For all suburbs in South Africa:
 * npx tsx server/scripts/extract_suburb_codes.ts
 */
const suburbCodeMap: Record<string, string> = loadSuburbCodes();
