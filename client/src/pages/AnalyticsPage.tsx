...
import { Building2, Loader2, BarChart3, FileText, Activity } from "lucide-react";
...
// API fetch function for PriceLabs usage stats
const fetchPriceLabsStats = async () => {
  const response = await fetch('/api/pricelabs-usage');
  if (!response.ok) {
    throw new Error('Failed to fetch PriceLabs usage stats');
  }
  return response.json();
};
...
const AnalyticsDashboard = () => {
  ...
  const { data: priceLabsStats, isLoading: priceLabsLoading } = useQuery({
    queryKey: ['pricelabs-usage'],
    queryFn: fetchPriceLabsStats,
    refetchInterval: 5 * 60 * 1000, // Refetch every 5 minutes
  });
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="w-full max-w-sm">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">PriceLabs API Calls</CardTitle>
          ...
          <CardContent>
            {priceLabsLoading ? (
              ...
              <div>
                <div className="text-2xl font-bold">{priceLabsStats?.totalCalls || 0}</div>
                <p className="text-xs text-muted-foreground">Total API calls made</p>
                {priceLabsStats?.monthlyUsage && priceLabsStats.monthlyUsage.length > 0 && (
                  <div className="mt-2">
                    <p className="text-xs text-muted-foreground">
                      This month: {priceLabsStats.monthlyUsage[0]?.apiCalls || 0} calls
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Success rate: {priceLabsStats.monthlyUsage[0]?.successfulCalls || 0}/{priceLabsStats.monthlyUsage[0]?.apiCalls || 0}
                    </p>
                    {priceLabsStats.monthlyUsage[0]?.avgResponseTime && (
                      <p className="text-xs text-muted-foreground">
                        Avg response: {priceLabsStats.monthlyUsage[0].avgResponseTime}ms
                      </p>
                    )}
                  </div>
                )}
