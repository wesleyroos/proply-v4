...
import { PropertyData } from "../types/propertyReport";
...
export async function generatePDF(
  ...
  try {
    ...
    const isSectionPopulated = <T extends Record<string, boolean>>(
      section: T,
    ): boolean => {
    ...
    const isSectionPopulatedFiltering = <T extends Record<string, boolean>>(
      obj: T,
      fieldToIgnore: keyof T,
    ): boolean => {
      return Object.entries(obj)
        .filter(([key]) => key !== fieldToIgnore) // Exclude the specified field
        .some(([_, value]) => value === true); // Check if any remaining value is true
    };
    ...
    const isTermPopulated = (
      rentalPerformance: Record<string, boolean>,
      searchString: string,
    ): boolean => {
      return Object.entries(rentalPerformance)
        .filter(([key]) => key.startsWith(searchString))
        .some(([_, value]) => value === true);
    };

    const countTrueFields = <T extends Record<string, boolean>>(obj: T): number => {
      return Object.values(obj).filter(value => value === true).length;
    };

    // Example Usage:
    // console.log(isSectionEmpty(reportSelections.cashflowMetrics)); // true or false
    // console.log(isSectionEmpty(reportSelections.financialMetrics)); // true or false
    ...
    console.log("ASDASDASD " + yPosition);
    ...
    const propertyDetails = [
      ...
      ...(selections.propertyOverview.propertyRatePerSquareMeter === true
        ? [
            [
              "Current Property Rate/m²",
              formatCurrency(
                data.propertyDetails.purchasePrice /
                  data.propertyDetails.floorArea,
              ),
            ],
          ]
        : []),
      ...(selections.propertyOverview.areaRatePerSquareMeter === true
        ? [
            [
              "Area Rate/m²",
              formatCurrency(data.propertyDetails.ratePerSquareMeter),
            ],
          ]
        : []),
      ...(selections.propertyOverview.rateDifference
        ? [
            [
              "Rate/m² Difference",
              formatCurrency(
    ...
    const requiredSpace = (10 * (countTrueFields(selections.financialMetrics) + countTrueFields(selections.operatingExpenses) + 11)); 
    console.log("Required space is " + requiredSpace)
    if (yPosition + requiredSpace > pageHeight - margin) {
      pdf.addPage();
      yPosition = margin;
    }
      ...
      ...(selections.financialMetrics.totalCapitalRequired
        ? [
            [
              ...
              formatCurrency(data.financialMetrics.totalCapitalRequired),
            ],
          ]
        : []),
    ];
    ...
    const operatingExpenses = [
      ["Monthly Levies", formatCurrency(data.expenses.monthlyLevies)],
      [
        "Monthly Rates and Taxes",
        formatCurrency(data.expenses.monthlyRatesTaxes),
      ],
      [
        "Other Monthly Expenses",
        formatCurrency(data.expenses.otherMonthlyExpenses),
      ],
    ...
    autoTable(pdf, {
      startY: yPosition,
      head: [["Metric", "Value"]],
      body: [...financialMetrics, ...operatingExpenses],
      margin: { left: margin },
      styles: {
        minCellHeight: 6,
        fontSize: 9,
    ...
    if (isTermPopulated(selections.rentalPerformance, "shortTerm")) {
      pdf.addPage();
      yPosition = margin;
      ...
      const shortTermPerformance = [
        ...
        ...(selections.rentalPerformance.shortTermAnnualRevenue
          ...
          : []),
        ...(selections.rentalPerformance.shortTermGrossYield
          ? [
              [
                "Gross Yield",
    ...
    if (isTermPopulated(selections.rentalPerformance, "longTerm")) {
      ...
      const longTermPerformance = [
        ...(selections.rentalPerformance.longTermMonthlyRevenue
          ? [
              [
                "Monthly Revenue",
        ...
        ...(selections.rentalPerformance.longTermAnnualRevenue
          ...
          : []),
        ...(selections.rentalPerformance.longTermGrossYield
          ? [
              [
                "Gross Yield",
    ...
    if (selections.rentalPerformance.rentalPerformanceChart === true) {
      ...
      monthlyPerformance.push([
        ...
        "45%",
        formatCurrency(
          monthlyPerformance[12][5].replace(/[^0-9.-]+/g, "") / 12,
        ),
        "65%",
        formatCurrency(
          monthlyPerformance[12][7].replace(/[^0-9.-]+/g, "") / 12,
        ),
        "85%",
        formatCurrency(
          monthlyPerformance[12][9].replace(/[^0-9.-]+/g, "") / 12,
        ),
        formatCurrency(data.performance.longTermAnnualRevenue / 12),
      ]);
    ...
    if (
      isSectionPopulatedFiltering(selections.cashflowMetrics, "cashflowChart")
    ) {
      pdf.setFontSize(16);
      pdf.setTextColor(0);
      ...
      const addCashflowMetricsTable = (
        ...
        const tableData = [
          ...(selections.cashflowMetrics.annualRevenue
            ? [
                [
                  "Annual Revenue",
                  ...years.map((year) => {
            ...
            : []),
          ...(selections.cashflowMetrics.netOperatingIncome
            ? [
                [
                  "Net Operating Income",
                  ...years.map((year) => {
                    const yearKey =
                      `year${year}` as keyof typeof operatingIncome;
                    return formatCurrency(
                      operatingIncome?.[yearKey]?.value || 0,
                    );
                  }),
                ],
              ]
            : []),
          ...(selections.cashflowMetrics.netOperatingExpense
            ? [
                [
          ...
          ...(selections.cashflowMetrics.annualBondPayment
            ? [
                [
                  ...
                  ...years.map(() =>
                    formatCurrency(
                      data.financialMetrics.monthlyBondRepayment * 12,
                    ),
                  ),
                ],
              ]
            : []),
          ...(selections.cashflowMetrics.annualCashflow
            ? [
                [
                  "Annual Cashflow",
                  ...years.map((year) => {
            ...
            : []),
          ...(selections.cashflowMetrics.cumulativeCashflow
            ? [
                [
                  "Cumulative Cashflow",
                  ...years.map((_, index) => {
                    ...
                    for (let i = 0; i <= index; i++) {
                      const y = years[i];
                      const yearKey =
                        `year${y}` as keyof typeof operatingIncome;
                      cumulative +=
                        operatingIncome?.[yearKey]?.annualCashflow || 0;
                    }
    ...
    if (selections.cashflowMetrics.cashflowChart === true) {
    ...
    if (isSectionPopulated(selections.investmentMetrics)) {
      ...
      const addInvestmentMetricsTable = (
        ...
        const tableData = [
          ...
          ...(selections.investmentMetrics.returnOnEquity
            ...
            : []),
          ...(selections.investmentMetrics.annualReturn
            ? [
                [
                  "Annual Return",
                  ...years.map((year, index) => {
          ...
          ...(selections.investmentMetrics.irr
            ...
            : []),
          ...(selections.investmentMetrics.netWorthChange
            ? [
                [
                  "Net Worth Change",
                  ...years.map((year, index) => {
    ...
    if (isSectionPopulated(selections.assetGrowthAndEquity)) {
    ...
    if (selections.assetGrowthAndEquity.assetGrowthAndEquityChart === true) {
    ...
    if (
      isSectionPopulatedFiltering(
        selections.assetGrowthAndEquity,
        "assetGrowthAndEquityChart",
      )
    ) {
      const tableDataAGE = [
        ...(selections.assetGrowthAndEquity.propertyValue
          ? [["Property Value", ...assetMetrics.map((m) => m[0])]]
          : []),
        ...(selections.assetGrowthAndEquity.annualAppreciation
          ? [["Annual Appreciation", ...assetMetrics.map((m) => m[1])]]
          : []),
        ...(selections.assetGrowthAndEquity.loanBalance
          ? [["Loan Balance", ...assetMetrics.map((m) => m[2])]]
          : []),
        ...(selections.assetGrowthAndEquity.totalInterestPaid
          ? [["Total Interest Paid", ...assetMetrics.map((m) => m[3])]]
          : []),
        ...(selections.assetGrowthAndEquity.interestToPrincipalRatio
          ? [["Interest-to-Principal Ratio", ...assetMetrics.map((m) => m[4])]]
          : []),
        ...(selections.assetGrowthAndEquity.totalEquity
          ? [["Total Equity", ...assetMetrics.map((m) => m[5])]]
          : []),
        ...(selections.assetGrowthAndEquity.loanRepaymentEquity
