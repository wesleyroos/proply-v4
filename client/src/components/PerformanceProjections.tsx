...
export default function PerformanceProjections({
  ...
  const cashflowData = years.map((year, index) => {
    const yearKey = `year${year}` as keyof typeof netOperatingIncome;
    const currentYearData = netOperatingIncome?.[yearKey];

    return {
      year: `Year ${year}`,
      'Annual Cashflow': currentYearData?.annualCashflow || 0,
      'Cumulative Cashflow': currentYearData?.cumulativeRentalIncome || 0,
    };
  });
